...
def ccache():
    ...
    return dict(
        form=form, total=total, ram=ram, disk=disk, object_stats=hp is not False
    )
...
def bg_graph_model():
    ...
    for tablename in db.tables:
        for field in db[tablename]:
            ...
            if isinstance(f_type, str) and (
                f_type.startswith("reference") or f_type.startswith("list:reference")
            ):
    ...
    if not request.args:
        ...
        response.headers[
            "Content-Disposition"
        ] = "attachment;filename=graph.%s" % request.args(0)
                ...
                for f in getattr(db[t], op):
                    if hasattr(f, "__call__"):
                        try:
                            ...
                            if details["filename"]:
                                # Built in functions as delete_uploaded_files
                                # are not editable
                                details["url"] = URL(
                                    a="admin",
                                    c="default",
